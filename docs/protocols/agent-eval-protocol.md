...
> **性质**：宪法追加条款（不改变既有条款语义）；对应 execution-plan-v1 §六.十六 L2。
> **背景**：G 窗口已取得"真实 LLM Agent 低分"证据（30.0 / L1×19，K 后口径）；
> L 窗口补"聚焦短分析"（决策点少 → 高分概率高）的真实 LLM 证据——高分不保证，
> 如实呈现（宪法 §6）。

### 10.1 任务设计（L2.5）

- **评测对象**：CellVoyager（同 §2，不 fork 不改码）；**运行 1 次**（n=1，随机性声明照旧）。
- **数据集**：GSE115978（同 §2，Smart-seq2，7,186 cells；平台事实 G-2 已查证定案）。
- **聚焦任务**：任务文件 `cellvoyager-outputs/windowL/task_gse115978_focused.txt`——
  仅做 QC + 归一化（+ 可选 HVG），**不做聚类/注释/轨迹/DEG**；范围限定是任务设计
  （决策点少），方法选择完全留给 Agent（审计对象不变）；终答 = 简短摘要
  （QC 前后细胞数 / QC 准则 / 归一化方法 / 最终维度）。
- **参数**：`max-iterations 4`（聚焦任务步数少，4 次迭代覆盖 QC→归一化→摘要；
  预算决策，非质量妥协）、`num-analyses 1`、`--no-deepresearch`、
  execution-mode=claude（同 §2）。
- **预算与停止**：**预算上限 ¥5**（余额差 + usage 换算双核算）、**超时上限 60 分钟**
  （正式运行启动起算）、API 重试 1 次（5s/15s 退避）、连续失败 3 次停止——
  全部与 §7 相同，**超限即停止并如实报告**。
- **采集与部署**：hook 复用 G 窗口已修路径（kwargs 提取器 / 双挂去重 /
  MCP 子进程 `_server_command` 注入）；密钥只环境变量注入；unset 继承代理。
- **报告**：分数/verdict/L 分布 + 每步耗时与 token（usage 换算）+ 成本
  （余额差权威口径）+ 与黄金对照/任务集描述性对比；**高分不保证**，若仍以
  L1 为主则如实归因（决策集小、方法学选择仍是真实风险）；失败/超限 →
  部分完成报告（宪法 §6/§7）。
