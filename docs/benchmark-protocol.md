# Bio-Audit Benchmark 协议（阶段 3 / 窗口 D）

> **版本**：v1（2026-08-16，与任务集 taskset v1.0.0 绑定）
> **依据**：refactor-plan-v1.1 E1-E8（评测方法论裁决）+ audit-report E 组 +
> execution-plan §六.七（D1-D6 验收清单，16 项冻结）
> **预注册记录**：`src/bioaudit/benchmark/pre_registration.json`（机器可读副本）
> **数据**：`src/bioaudit/data/tasks/`（30 条，批 1/2）+ `src/bioaudit/data/annotation/`
...
- **规模**：目标 ≥60 条（3 范式 × 难度梯度 × 类型覆盖）。**首批 30 条已落地**
  （scrna 12 / pan 10 / deg 8）；批 2（30 条）排期补齐（见 §九）。
- **格式**：每条 = v2 轨迹（version/trajectory_id/provenance/decisions）+
  `gold`（E3 双标注产物）+ `difficulty`（E4 特征 rubric 产物）。
- **版本管理**（E8）：`taskset.json` semver（当前 1.0.0）+ 文件级 SHA256 +
  快照三元组（engine/ruleset/ontology，C1/P2）；任务/标注变更走
...
- 实测：见 `data/annotation/irr_report.json` 与完成报告。
...
1. **批 2（30 条 → 60 条）**：排期（下一执行窗口）；流程不变（语料扩展 +
   generator_prompt.md v1 规格 → 校准批 IRR → 放量）。
2. **同模型双标注局限**：两个标注员为同族 LLM（独立上下文）——IRR 可能
   高估；跨模型标注（如人工 + LLM）列为批 2 改进项。
3. CellVoyager 真实运行语料随阶段 2 遗留（hook 未实测），新错误模式素材
   待真实运行补充。
